/**
 * Sistema de Questões Interativas FINAL - AlfaReview Geografia
 * Versão corrigida que usa o banco de questões integrado
 */
...
class InteractiveQuestionsSystem {
    ...
    init() {
        ...
        // Aguardar carregamento do banco de questões
        this.waitForDatabase();
    }
    
    /**
     * Aguarda o carregamento do banco de questões
     */
    waitForDatabase() {
        if (typeof window.QUESTIONS_DATABASE !== 'undefined') {
            console.log('Banco de questões disponível!');
        } else {
            console.log('Aguardando banco de questões...');
            setTimeout(() => this.waitForDatabase(), 100);
        }
    }
    ...
    startQuestions(unitId) {
        if (typeof window.QUESTIONS_DATABASE === 'undefined') {
            console.error('Banco de questões não carregado!');
            this.showMessage('Aguarde o carregamento das questões...');
            setTimeout(() => this.startQuestions(unitId), 1000);
            return;
        }
        ...
        // Carregar questões da unidade do banco integrado
        this.questions = window.QUESTIONS_DATABASE[`unidade_${unitId}`] || [];
        
        if (this.questions.length === 0) {
            this.showMessage(`Questões não encontradas para a Unidade ${unitId}.`);
            return;
        }
        ...
        modal.innerHTML = `
            <div class="feedback-content unit-${this.currentUnit}">
                ...
                <div class="stitch-feedback">
                    <img src="upload/Unidade${this.currentUnit}.png" 
        ...
        panel.innerHTML = `
            <div class="review-content">
                ...
                <div class="review-body" id="review-body">
                    <img src="upload/mapa${this.currentUnit === 6 ? 'Unidade6-MatrizEnergéticaBrasileira_FontesRenováveiseNãoRenováveis' : 'mental_Unidade' + this.currentUnit}.png" 
